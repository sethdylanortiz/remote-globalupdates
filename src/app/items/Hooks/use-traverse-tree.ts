...
const useTraverseTree = () => {
    ...
    // todo: change from tree.name == parent to id!!
    const insertNode = (tree: any, parentId: number, newItemName: string, newItemValue: string, isFolder: boolean) => {

        // check if we're at desired sub-tree, confirm if subree is folder
        if((tree.__id == parentId) && (tree.__isFolder == true)) 
        {
            if(isFolder == true)
            {
        ...
        let currentNode = tree.__items.map((item: any) => {
            return insertNode(item, parentId, newItemName, newItemValue, isFolder);
        });
